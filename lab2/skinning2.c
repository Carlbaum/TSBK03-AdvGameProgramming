...
void BuildCylinder()
{
  ...
  // s�tter v�rden till alla vetexar i meshen
  for (row = 0; row < kMaxRow; row++)
    for (corner = 0; corner < kMaxCorners; corner++)
      {
...
void setupBones(void)
{
  ...
  for (bone = 0; bone < kMaxBones; bone++) {
    ...
    local_coords[bone] = bone > 0 ?  Mult(T(g_bones[bone-1].pos.x - g_bones[bone].pos.x,0,0),local_coords[bone-1]) : T(-g_bones[bone].pos.x,0,0);
    printMat4(local_coords[bone]);
  }
}
...
void DeformCylinder()
{
  ...
  for (row = 0; row < kMaxRow; row++)
  {
    for (corner = 0; corner < kMaxCorners; corner++)
    {
      ...
      // g_vertsOrg
      // g_vertsRes
      // mat4 M_prim[kMaxBones];
      // mat4 M_inv[kMaxBones];
      // mat4 M[kMaxBones];
      // for(int bone = 0; bone < kMaxBones; bone++) {
      //   M_prim[bone] = Mult(T(g_bones[bone].pos.x,g_bones[bone].pos.y,g_bones[bone].pos.z),g_bonesRes[bone].rot);
      //   M_inv[bone] = T(-g_bones[bone].pos.x, -g_bones[bone].pos.y, -g_bones[bone].pos.z);
        // printf("M_inv[%d] = \n",0);
        // printf("(%f,%f,%f,%f)\n", g_bonesRes[0].rot.m[0] , g_bonesRes[0].rot.m[1],g_bonesRes[0].rot.m[2] ,g_bonesRes[0].rot.m[3]);
        // printf("(%f,%f,%f,%f)\n", g_bonesRes[0].rot.m[4] , g_bonesRes[0].rot.m[5],g_bonesRes[0].rot.m[6] ,g_bonesRes[0].rot.m[7]);
        // printf("(%f,%f,%f,%f)\n", g_bonesRes[0].rot.m[8] , g_bonesRes[0].rot.m[9],g_bonesRes[0].rot.m[10],g_bonesRes[0].rot.m[11]);
        // printf("(%f,%f,%f,%f)\n", g_bonesRes[0].rot.m[12],g_bonesRes[0].rot.m[13],g_bonesRes[0].rot.m[14],g_bonesRes[0].rot.m[15]);
      //   // printf("\nM_prim[%d] = \n",bone);
      //   // printf("(%f,%f,%f,%f)\n", M_prim[bone].m[0] , M_prim[bone].m[1],M_prim[bone].m[2] ,M_prim[bone].m[3]);
      //   // printf("(%f,%f,%f,%f)\n", M_prim[bone].m[4] , M_prim[bone].m[5],M_prim[bone].m[6] ,M_prim[bone].m[7]);
      //   // printf("(%f,%f,%f,%f)\n", M_prim[bone].m[8] , M_prim[bone].m[9],M_prim[bone].m[10],M_prim[bone].m[11]);
      //   // printf("(%f,%f,%f,%f)\n\n\n\n", M_prim[bone].m[12],M_prim[bone].m[13],M_prim[bone].m[14],M_prim[bone].m[15]);
      // }
      // vec3 v_tot = SetVector(0,0,0);
      // for(int bone = 0; bone < kMaxBones; bone++) {
      //   mat4 tmp_prim;
      //   mat4 tmp_inv;
      //   for(int i = 0; i <= bone; i++) {
      //     if(i>0) {
      //       tmp_prim = Mult(tmp_prim, M_prim[i]);
      //       tmp_inv = Mult(M_inv[i], tmp_inv);
      //     } else {
      //       tmp_prim = M_prim[i];
      //       tmp_inv = M_inv[i];
      //     }
      //   }
      //   M[bone] = Mult(tmp_prim,tmp_inv);
      //   v_tot =  VectorAdd(v_tot,ScalarMult(MultVec3(M[bone],g_vertsOrg[row][corner]),g_boneWeights[row][corner][bone]));
      // }
      // g_vertsRes[row][corner] = v_tot;
      mat4 modified_transform[kMaxBones];
      vec3 v_accumulator = SetVector(0,0,0);

      for(int bone = 0; bone < kMaxBones; bone++) {
        mat4 tmp = Mult(T(4,0,0), g_bonesRes[bone].rot); // calculates current M'
        modified_transform[bone] = bone > 0 ? Mult(modified_transform[bone-1] ,tmp) : Mult(T(0,0,0),g_bonesRes[bone].rot);
        v_accumulator = VectorAdd(v_accumulator,ScalarMult(MultVec3(Mult(modified_transform[bone],local_coords[bone]),g_vertsOrg[row][corner]),g_boneWeights[row][corner][bone]));
      }
      g_vertsRes[row][corner] = v_accumulator;
    }
  }
}
